...
class AffConnection(SageObject):
    ...
    def _derive_paral(self, tensor):
        ...
        print "MMARCO : covariant derivative"
        marco_t0 = time.time()
        if manifoldPara.use_paral :
            ...
            for ind_part in local_list:
                listParalInput.append((ind_part,tc,gam,frame,n_con,tensor,manif))

            # definition of the parallel function
            @parallel(p_iter='multiprocessing',ncpus=manifoldPara.nproc)
            def make_CovDerivative(ind_part,tc,gam,frame,n_con,tensor,manif):
                partial = []
                for ind in ind_part:
